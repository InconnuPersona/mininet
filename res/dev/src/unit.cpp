...
void createunit(unit_t* unit, const char* word) {
 ...
 LOGDEBUG(2, "created unit from word '%s' with id [%x] and word [%x].", word, unit->id, unit->word);
...
refer_t designunit(const char* word, int extra) {
 ...
 if (slot != -1) {
  ...
  LOGDEBUG(2, "defined unit '%s' (%s) with id [%x] in %i elements.", unitwords[slot].word, unitwords[slot].name, unitwords[slot].id, length + extra);
 }
 else {
