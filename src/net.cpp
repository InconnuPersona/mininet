...
void closehost() {
 ...
 memset(&clients, 0, (MAX_CLIENTS + 1) * sizeof(client_t));
...
void packheader(short type, int flags, int length) {
 ...
 header->length = transmission->len = sizeof(packetheader_t) + length;
...
void receivepackets() {
 ...
 while (SDLNet_UDP_Recv(host.socket, transmission) > 0) {
  LOGDEBUG(3, "received packet [%i].", packet.type);
  
  unpackheader(&packet);
  
  if (packet.length + sizeof(packetheader_t) != transmission->len) {
   LOGREPORT("received nonuniform packet lengths; packet length [%i] vs. UDP packet length [%i].", packet.length + sizeof(packetheader_t), transmission->len);
   continue;
  }
  
  data = (int*) (transmission->data + sizeof(packetheader_t));
  
  if (packet.type == NETMSG_HANDSHAKE) {
   receivehandshake(data[0], data[1], &packet, transmission->address);
  }
...
void unpackheader(packet_t* packet) {
 ...
 packet->length = header->length - sizeof(packetheader_t);
